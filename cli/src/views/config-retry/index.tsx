...
export default function ConfigRetryPage() {
  ...
  const [inputValue, setInputValue] = useState(String(config.concurrency.max_retries));
  ...
  useInput((input, key) => {
    ...
    if (key.return && saveStatus === 'idle') {
      if (validate(inputValue)) {
        ...
        setField('concurrency.max_retries', num);
        navigate('/config');
      }
    }
    // s 键保存并返回
    ...
    <Box flexDirection="column">
      ...
      <Box marginTop={1}>
        ...
        <TextInput
          ...
          placeholder={String(config.concurrency.max_retries)}
        />
      </Box>

      {/* 范围提示 */}
      <Box marginTop={1}>
        <Text dimColor>{t('retry.range')}</Text>
      </Box>
      ...
      {/* Footer */}
      <Box marginTop={1}>
        <Text dimColor>{t('retry.footer')}</Text>
      </Box>
    </Box>
  );
}