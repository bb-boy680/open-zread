...
interface OpenAIChatMessage {
  ...
  tool_call_id?: string
  // DeepSeek thinking mode: reasoning content must be passed back
...
export class OpenAIProvider implements LLMProvider {
  ...
  private convertAssistantMessage(
    ...
    // Extract text, tool_use, and reasoning/thinking blocks
    const textParts: string[] = []
    ...
    for (const block of msg.content) {
      if (block.type === 'text') {
        ...
        // DeepSeek thinking mode: preserve reasoning content for subsequent requests
        reasoningContent = block.reasoning
      } else if (block.type === 'thinking') {
        // Thinking block is converted from reasoning, treat as reasoning_content
    ...
    // DeepSeek thinking mode: pass back reasoning content
    if (reasoningContent) {
  ...
  private convertResponse(data: OpenAIChatResponse): CreateMessageResponse {
    ...
    // DeepSeek thinking mode: add reasoning content first (if present)
    if (choice.message.reasoning_content) {
      content.push({ type: 'reasoning', reasoning: choice.message.reasoning_content })
    }
